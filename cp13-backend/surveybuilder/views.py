...
from surveybuilder.models import Survey, Randomiser, Block, Question, SocialPostQuestion, ButtonRowQuestion, ButtonQuestion, TextboxQuestionNumber, TextboxQuestionText, MultiChoiceQuestion, MultiChoice, NumberScaleQuestion, DragAndDropQuestion, DragAndDropChoice, DragAndDropCard
from surveybuilder.serializers import SurveySerializer, RandomiserSerializer, BlockSerializer, QuestionSerializer, SocialPostQuestionSerializer, ButtonRowQuestionSerializer, ButtonQuestionSerializer, TextboxQuestionNumberSerializer, TextboxQuestionTextSerializer, MultiChoiceSerializer, MultiChoiceQuestionSerializer, NumberScaleQuestionSerializer, DragAndDropChoiceSerializer, DragAndDropQuestionSerializer, DragAndDropCardSerializer
...
def survey_data(request, id):
    ...
    try: 
        ...
        randomisers = Randomiser.objects.filter(survey=survey.id)
    ...
    randomisers_serilized = RandomiserSerializer(randomisers, many=True)
    # Extract data
    survey_data = survey_serialized.data
    survey_data['blocks'] = blocks_serialized.data[:]
    survey_data['randomiser'] = randomisers_serilized.data[:]
...
########## Randomiser related API function calls ##########
# Require a post, delete, patch, get methods

@api_view(['GET', 'PATCH', 'DELETE'])
def randomiser_info(request, id):
    """
    get:
    Get a randomiser by its ID

    post:
    Add a randomiser by its ID

    delete:
    Remove a randomiser by its ID
    """
    try: 
        random = Randomiser.objects.get(pk=id)
    except Randomiser.DoesNotExist: 
        return JsonResponse({'Message': 'The randomiser element can\'t be found.'}, status=status.HTTP_404_NOT_FOUND) 
 
    if request.method == 'GET':
        # GET a specific randomiser element by ID

        random_serialised = RamdomiserSerializer(random)
        return JsonResponse(random_serialised.data)

    elif request.method == 'DELETE':
        # DELETE a specific randomiser element by ID

        survey.delete()
        return JsonResponse({'Message': 'The survey has been deleted.'}, status=status.HTTP_204_NO_CONTENT)

    elif request.method == 'POST':
        # UPDATE a specific randomiser element by ID
      
        parsed_request = JSONParser().parse(request)
        # Needs to determine if adding new blcok to the relation or just 

        if random_serialised.is_valid():
            random_serialised.save()
            return JsonResponse(random_serialised.data) 
        return JsonResponse(random_serialised.errors, status=status.HTTP_400_BAD_REQUEST)
...
def block_info(request, id):
    ...
    if request.method == 'GET':
        ...
        if "randomiser" in parsed_request.keys():
            parsed_request["randomiser"] = Randomiser.objects.get(pk = parsed_request["randomiser"]).id
...
@api_view(['GET', 'POST'])
def randomiser_list(request, id):
    """
    get:
    Get all randomisers within a survey

    post:
    Include a randomiser to a given survey
    """
    try: 
        # Needs to include some sort of authentication check here
        survey = Survey.objects.get(pk=id)
    ...
    if request.method == 'GET':
        # GET list of randomisers in a given survey
        
        randomisers = Randomiser.objects.filter(survey=survey.id)
        randomisers_serialized = RandomiserSerializer(randomisers, many=True)
        return JsonResponse(randomisers_serialized.data, safe=False)

    elif request.method == 'POST':
        # POST a new randomiser
        
        parsed_request = JSONParser().parse(request)
        parsed_request["survey"] = survey.id

        randomisers_serialized = RandomiserSerializer(data = parsed_request)
        
        if randomisers_serialized.is_valid():
            randomisers_serialized.save()
            return JsonResponse(randomisers_serialized.data, status=status.HTTP_201_CREATED) 
        return JsonResponse(randomisers_serialized.errors, status=status.HTTP_400_BAD_REQUEST)

@api_view(['GET', 'DELETE', 'PATCH'])
def randomiser_info(request, id):
    """
    get:
    Get a specific randomiser information by ID

    delete:
    Delete a specific randomiser by ID

    patch:
    Update a randomiser information
    """
    try:
        randomiser = Randomiser.objects.get(pk = id)
    except Randomiser.DoesNotExist:
        return JsonResponse({'Message': 'The randomiser can\'t be found.'}, status=status.HTTP_404_NOT_FOUND) 

    if request.method == 'GET':
        # GET a specific randomiser
        randomiser_serialized = RandomiserSerializer(randomiser)
        return JsonResponse(randomiser_serialized.data)

    elif request.method == 'DELETE':
        # DELETE a specific randomiser
        randomiser.delete()
        return JsonResponse({'Message': 'The randomiser has been deleted.'}, status=status.HTTP_204_NO_CONTENT)

    elif request.method == 'PATCH':
        # Update a specific randomiser
        parsed_request = JSONParser().parse(request)
        randomiser_serialized = RandomiserSerializer(randomiser, data=parsed_request, partial=True)
        
        if randomiser_serialized.is_valid():
            randomiser_serialized.save()
            return JsonResponse(randomiser_serialized.data) 
        return JsonResponse(randomiser_serialized.errors, status=status.HTTP_400_BAD_REQUEST) 

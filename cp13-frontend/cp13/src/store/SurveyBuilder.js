...
export default new Vuex.Store({
    ...
    mutations: {
         ...
         */
        updateStartWith(state, {
            oldStart,
            newStart
        }) {
            // Find index of section in randomSections
            let index = state.survey.editorData.randomSections.indexOf(state.survey.editorData.randomSections.filter(e => e.startWith == oldStart)[0])

            // Update startWith to newStart
            state.survey.editorData.randomSections[index].startWith = newStart

            // Make sure display is in range
            let difference = state.survey.editorData.randomSections[index].endWith - state.survey.editorData.randomSections[index].startWith + 1
        ...
        async deleteBlock(state, block) {
            ...
            for (let i = 0; i < state.survey.editorData.randomSections.length; i++) {
                ...
                if (state.survey.editorData.randomSections[i].endWith < state.survey.editorData.randomSections[i].startWith) {
            ...
            // Update order of all blocks following the new one, by descending order
            for (
                let i = order + 1; i <= state.survey.editorData.blocks.length + 1; i++
            ) {
         ...
         */
        async duplicateQuestion(state, {
            block,
            question
        }) {
            let sortedQuestions = block.questionData.questions.slice().sort(function compare(a, b) {
                if (a.order < b.order) {
                    return -1;
                }
                return 1;
            ...
            for (let i = block.questionData.questions.length - 1; i >= question.order; i--) {
        ...
        async deleteQuestion(state, {
            ...
            for (
                let i = order + 1; i <= block.questionData.questions.length + 1; i++
            ) {
         ...
         */
        handleTextAnswerType(state, {
            question,
            ansType
        }) {
            if (question.answerType != ansType) {
                switch (ansType) {
                    case "Text":
                        ...
                        Vue.set(question, 'answerType', 'Text');
                        break
                    case "Integer":
                        Vue.set(question, 'type', "Text entry");
                        Vue.set(question, 'query', "");
                        Vue.set(question, 'textBoxMax', 100);
                        Vue.set(question, 'textBoxMin', 0);
                        Vue.set(question, 'answerType', 'Integer');
                        break
                    case "Decimal":
                        Vue.set(question, 'type', "Text entry");
                        Vue.set(question, 'query', "");
                        Vue.set(question, 'textBoxMax', 100);
                        Vue.set(question, 'textBoxMin', 0);
                        Vue.set(question, 'answerType', 'Decimal');
                }
            } else {
                return;
            }
            question.answerType = ansType;
        },
        /**
         ...
         */
        async handleQuestionType(state, {
            oldquestion,
            type,
        }) {
            console.log(state)
            ...
            let thisBlock = state.survey.editorData.blocks.filter(e => e.id == block_id)[0]
            thisBlock.questionData.questions.splice(thisBlock.questionData.questions.indexOf(oldquestion), 1, newQuestion)
        ...
        async insertNewQuestion(state, {
            ...
            switch (type) {
                ...
                case "Multiple choice":
                    ...
                    break;
                case "Number scale":
                    Vue.set(newQuestion, 'minNum', 1);
                    Vue.set(newQuestion, 'maxNum', 5);
                    Vue.set(newQuestion, 'interval', 1);
                    Vue.set(newQuestion, 'minTitle', i18n.t('app.numScaleNot'));
                    Vue.set(newQuestion, 'midTitle', i18n.t('app.numScaleSomewhat'));
                    Vue.set(newQuestion, 'maxTitle', i18n.t('app.numScaleVery'));
                    Vue.set(newQuestion, 'minTitleOn', true);
                    Vue.set(newQuestion, 'midTitleOn', true);
                    Vue.set(newQuestion, 'maxTitleOn', true);
                    break;
                case "Drag and Drop":
        ...
        incrementDisplay(state, start) {
            let thisSection = state.survey.editorData.randomSections.filter(e => e.startWith == start)[0]
            if (thisSection.display < (thisSection.endWith - thisSection.startWith + 1)) {
                thisSection.display++
        ...
        removeRandomSection(state, order) {
            let index = state.survey.editorData.randomSections.indexOf(state.survey.editorData.randomSections.filter(section => section.startWith == order)[0])
         ...
         */
        async insertQuestionDragDrop(survey, {
            order,
            block,
            question,
            oldBlock,
            ...
            if (oldBlock === block){
                ...
                // update order of all questions in new block
                for (let i = block.questionData.questions.length-1; i >= question.order-1; i--) {
                    sortedQuestions[i].order++;
                    await SurveyServices.patchQuestion(sortedQuestions[i].id, {
                      order: sortedQuestions[i].order
                    })
         ...
         */
        async deleteQuestionDragDrop(survey, {
            oldOrder,
            oldBlock,
            question,
            newBlock
            ...
            let sortedQuestions = oldBlock.questionData.questions.slice().sort(function compare(a, b) {
                if (a.order < b.order) {
                    return -1;
                }
                return 1;
            ...
            for (
                let i = oldOrder; i < oldBlock.questionData.questions.length; i++
            ) {
    ...
    actions: {
        ...
        async loadSurvey({
            ...
            for (var i = 0; i < response.blocks.length; i++) {
                ...
                for (var k = 0; k < response.blocks[i].questionData.questions.length; k++) {
                    ...
                    response.blocks[i].questionData.questions[k].title = response.blocks[i].questionData.questions[k].name

                    if (response.blocks[i].questionData.questions[k].type == "Text entry") {
                        response.blocks[i].questionData.questions[k].answerType = 'Text'
                    }

                    if (response.blocks[i].questionData.questions[k].type == "Number entry") {
                        response.blocks[i].questionData.questions[k].answerType = 'Integer'
                    }

                    if (response.blocks[i].questionData.questions[k].type == "News post") {
                        response.blocks[i].questionData.questions[k].articleImage = response.blocks[i].questionData.questions[k].articleImageLink
                        response.blocks[i].questionData.questions[k].likeCount = response.blocks[i].questionData.questions[k].articleLikes
                        response.blocks[i].questionData.questions[k].shareCount = response.blocks[i].questionData.questions[k].articleShares
                        response.blocks[i].questionData.questions[k].commentCount = response.blocks[i].questionData.questions[k].articleComments
                    }

                    if (response.blocks[i].questionData.questions[k].type == "Multiple choice") {
                        for (var y = 0; y < response.blocks[i].questionData.questions[k].choices.length; y++) {
                            response.blocks[i].questionData.questions[k].choices[y].text = response.blocks[i].questionData.questions[k].choices[y].title
                            response.blocks[i].questionData.questions[k].choices.sort((a, b) => (a.order - b.order) ? 1 : ((b.order - a.order) ? -1 : 0))
                        }
                    }

                    if (response.blocks[i].questionData.questions[k].type == "Number scale") {
                        response.blocks[i].questionData.questions[k].minNum = response.blocks[i].questionData.questions[k].numberMin
                        response.blocks[i].questionData.questions[k].maxNum = response.blocks[i].questionData.questions[k].numberMax
                        response.blocks[i].questionData.questions[k].midTitle = response.blocks[i].questionData.questions[k].middleTitle
                        response.blocks[i].questionData.questions[k].minTitleOn = true 
                        response.blocks[i].questionData.questions[k].maxTitleOn = true
                        response.blocks[i].questionData.questions[k].midTitleOn = true
                    ...
                    if (response.blocks[i].questionData.questions[k].type == "Button row") {
                        // response.blocks[i].questionData.questions[k].goToEnd = false
                        response.blocks[i].questionData.questions[k].terminating = response.blocks[i].questionData.questions[k].goToEnd
                        for (var q = 0; q < response.blocks[i].questionData.questions[k].buttons.length; q++) {
                            response.blocks[i].questionData.questions[k].buttons[q].text = response.blocks[i].questionData.questions[k].buttons[q].buttonText
                            // response.blocks[i].questionData.questions[k].buttons[q].goToEnd = false
                            response.blocks[i].questionData.questions[k].buttons[q].terminates = response.blocks[i].questionData.questions[k].buttons[q].goToEnd
                            response.blocks[i].questionData.questions[k].buttons.sort((a, b) => (a.order - b.order) ? 1 : ((b.order - a.order) ? -1 : 0))
                        }
                    }
                }
        ...
        async duplicateBlock({
            ...
            if (blockOrder != null) {
                thisBlock = getters.blocks.filter(block => block.order == blockOrder)[0]
            ...
            for (let i = 0; i < state.survey.editorData.randomSections.length; i++) {
                if (state.survey.editorData.randomSections[i].endWith >= duplicate.order) {
                    state.survey.editorData.randomSections[i].endWith++;
                }

                if (state.survey.editorData.randomSections[i].startWith > duplicate.order) {
                    state.survey.editorData.randomSections[i].startWith++;
                }
        ...
        async clearBlock({
            ...
            getters.currentBlock.questionData.questions.map(async (question) => await SurveyServices.deleteQuestion(question.id))
    ...
    getters: {
        ...
        randomSections: state => {
            ...
            // console.log(state.survey.editorData.randomSections.slice().sort(compare))
        ...
        sortedQuestions: state => index => {
            ...
            return state.survey.editorData.blocks[index].questionData.questions.slice().sort(compare);
        },
        currentBlock: state => {
